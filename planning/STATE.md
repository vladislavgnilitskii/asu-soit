...
> **Обновление 2026-07-02 — выполнены Этапы 0, 1, 2 и 3.** Закрыты все security-дыры, устранён тех-долг, миграция 009 применена, добавлены модуль устройств, полноценные организации, полный жизненный цикл заявки и модуль склада (запчасти, приход/списание, выдача в ремонт). Сборка/vet/тесты проходят (Go 1.26.4). Решения — в `DECISIONS.md`, следующая задача — в `CURRENT_TASK.md`.
...
- ✅ **Заявки end-to-end** — назначение исполнителя, диагностика/стоимость, закрытие (`closed_at`), чтение истории статусов реализованы (Этап 2). Связка с запчастями теперь есть (Этап 3); остаётся связка со счётом (`invoices`) — модуль финансов.
