...
> **Обновление 2026-07-02 — выполнены «Этап 0: Стабилизация» и «Этап 1: Клиенты-организации + Devices».** Закрыты все security-дыры, устранён тех-долг, миграция 009 применена, добавлен модуль устройств и полноценные организации. Сборка/vet/тесты проходят (Go 1.26.4). Решения — в `DECISIONS.md`, следующая задача — в `CURRENT_TASK.md`.
...
- **Заявки** — нет назначения инженера (`assigned_to`), диагностики/стоимости, закрытия (`closed_at`), чтения истории статусов. Устройство для заявки теперь можно создать через модуль Devices.
