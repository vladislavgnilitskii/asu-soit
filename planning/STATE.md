...
> **Обновление 2026-07-09 — фронт (дизайн-проход) + hardening бэкенда.** Фронт: «оливково-чёрная» тема (токены shadcn в oklch), тёмный режим с переключателем (`lib/theme.tsx`), скелетоны загрузки (`ui/skeleton`, `TableSkeleton`), полировка дашборда/логина, code splitting по маршрутам (`React.lazy`; стартовый бандл 527→353 kB). Бэкенд-hardening (без изменения схемы/архитектуры): rate-limit логина (10/мин на IP, `auth.LoginRateLimiter`), заголовки безопасности + строгий CSP (`auth.SecurityHeaders`), лимит тела 1 МиБ (`auth.BodyLimit`), graceful shutdown + HTTP-таймауты в `cmd/server/main.go`, health/readiness-пробы (`/api/v1/health`, `/health/ready`, `handler/health.go`). Закрыт техдолг D-031: `rows.Err()` во всех старых list-методах. Всё проверено рантаймом под `app_backend` (headers, 429, health, graceful stop); build/vet/тесты зелёные. Решения — D-032 в `DECISIONS.md`. **Пагинация (2026-07-10, D-033):** offset-пагинация `{items,total,limit,offset}` на списках заявок/клиентов/устройств/склада/сотрудников (`?limit&offset`, потолок 100, `total` RLS-aware) + `GET /requests/stats` для дашборда; фронт — компонент `Pagination` + `keepPreviousData`. **Осталось до прода (требует решения пользователя):** раннер миграций (golang-migrate vs своя реализация), self-service смена пароля (нужны права БД / SECURITY DEFINER), инфра (Dockerfile фронта/nginx, CI, мониторинг).
>
> **Обновление 2026-07-08 — выполнены Этапы 0–5 + начата безопасность БД (курсовая).** Реализованы все функциональные модули (клиенты, устройства, заявки, склад, финансы, сотрудники). Начат отдельный трек **безопасности БД** — см. `planning/DB_SECURITY.md` (анализ нормализации + модули). Проведена нормализация до строгой 3NF (миграция 011: удалён избыточный `repair_requests.client_id`). Безопасность БД: Модуль 1 (роли + least-privilege GRANT, миграция 010), Модуль 2 (RLS на заявках, миграция 012), Модуль 3 (представления + маскирование PII, миграция 013), Модуль 4 (триггеры аудита в audit_log, миграция 014) — сделаны и проверены в psql. Проброс личности из JWT в сессию БД реализован (middleware `auth.TxPerRequest` + пакет `internal/dbtx`, транзакция-на-запрос). Модуль 5: приложение переведено на непривилегированную роль `app_backend` с `SET ROLE` на каждый запрос (миграция 015) — теперь GRANT и RLS действуют и для приложения; логин через SECURITY DEFINER-функцию `auth_get_credentials`; RBAC-маршруты ужесточены под гранты ролей. Проверено рантаймом под `app_backend`. `.env.example` → `DB_USER=app_backend`. Модуль 6: паспортные данные шифруются pgcrypto (`bytea`, `pgp_sym_encrypt`, миграция 016), ключ — в приложении (`env CRYPTO_KEY`), не в БД; витрина `v_individuals` паспорт не раскрывает. Модуль 7: защита от SQL-инъекций (весь SQL на `$1`, контр-пример в psql) — задокументирована. **Все 7 запланированных модулей безопасности БД готовы и проверены** (см. `DB_SECURITY.md`). Возможное развитие: перенос проверки пароля в БД (`crypt()`), RLS на другие таблицы, ротация ключа, CRUD паспортных данных с шифрованием на стороне приложения. Сборка/vet/тесты проходят (Go 1.26.4). Решения — в `DECISIONS.md`.
...
- **Frontend** — основной функционал готов (`frontend/`): Vite + React + TypeScript + Tailwind v4 + shadcn/ui (**Base UI** `@base-ui/react`, не Radix — триггеры через `render`, `onValueChange` даёт `string|null`, селектам нужен `items=[{value,label}]`, иначе показывают сырое value — был баг с UUID вместо названия статуса, исправлен), TanStack Query, React Router. Экраны: вход, обзор, **заявки** (список RLS-aware, клик → карточка), **карточка заявки** (назначение инженера через `GET /engineers` ← витрина `v_employees`; диагностика/стоимость; выдача запчастей; история; закрытие; счёт — выставить/оплатить/отменить), **клиенты**, **устройства**, **склад** (позиции/приход/списание), **сотрудники** (admin: создание, вкл/выкл). Роли гейтят действия на клиенте поверх серверного RBAC/RLS. Полный жизненный цикл прогнан e2e против докерной БД (все 200/201). Бэкенд-дополнения: `GET /engineers` (admin/manager), миграция 017 (грант admin на v_employees), `rows.Err()` в новых list-методах. **Техдолг:** старые list-методы репозиториев не проверяют `rows.Err()` — ошибка выполнения запроса выглядит как пустой список. Dev-сид: все 5 ролей, пароль `admin123`. Для обновления стенда нужен `docker compose up --build` (оба образа: backend-код и db-миграции).
- **Инфраструктура** — начата частично: `docker-compose.yml` (Postgres + backend) + `backend/Dockerfile` (multi-stage, distroless) + `db/seed/dev_seed.sql` для локального стенда одной командой. K8s/Prometheus/Grafana — не начаты. Фронт-сервис в compose пока не добавлен (запускается через `npm run dev`, добавим когда UI устаканится). Устранена циклическая зависимость миграций 005↔006 (`request_parts` перенесён в 006) — набор теперь встаёт с нуля начисто (проверено локально: все 16 миграций + сид в порядке 001→016). Postgres в compose собирается из `db/Dockerfile` (COPY миграций+сида в initdb — надёжнее вложенного тома). Не проверено самим docker-рантаймом: демон в среде недоступен — валиден синтаксис (`docker compose config`) и чистое применение миграций в локальном psql.
