...
> **Обновление 2026-07-02 — «Этап 0: Стабилизация» выполнен.** Закрыты все security-дыры из разделов 5.1–5.3, устранена часть тех-долга и добавлен фундамент тестов. Сборка/vet/тесты проходят (Go 1.26.4). Подробности — в блоке «Итог Этапа 0» ниже; закрытые пункты помечены ✅. Матрица ролей и прочие решения — в `DECISIONS.md`, текущая/следующая задача — в `CURRENT_TASK.md`.
...
- **Клиенты — только individual.** `CreateClientRequest` содержит поля физлица (`last_name`/`first_name`/`middle_name`), но не содержит полей организации (`name`/`inn`/`kpp`). `ClientRepository.Create` для типа `organization` пишет только в `clients` и **не пишет** в `organizations` → создаётся orphan-запись (см. п.5). Чтения `individuals`/`organizations` в API нет вовсе — вернуть ФИО/название клиента нельзя.
- **RBAC.** Роли заведены в БД (6 шт.), но `RequireRole` применён только к `clients POST` (`admin`). Все эндпоинты заявок доступны любому авторизованному без учёта роли.
- **История статусов.** Пишется, но автор берётся из клиентского заголовка `X-Employee-ID`, а не из JWT (см. п.5, TODO в коде).

---

## 3. Что отсутствует

- **Handler/Repository для Devices** — таблицы `devices`/`device_types` есть, кода нет.
...
7. ⏳ **Остаётся:** при смене статуса не проверяется существование `status_id`/заявки до вставки — несуществующий `status_id` даст FK-ошибку и 500 вместо 400/404. Кандидат в следующий этап.
...
- **Миграция 009** — файл готов, но **к живой БД не применён**; требует подтверждения (изменение схемы).
