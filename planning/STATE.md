...
- **Frontend** — в работе (`frontend/`): Vite + React + TypeScript + Tailwind v4 + shadcn/ui (**Base UI** `@base-ui/react`, не Radix — триггеры через `render`, `onValueChange` даёт `string|null`), TanStack Query, React Router. Реализованы: вход (JWT в localStorage, роль из claims), оболочка с ролевым меню, обзор; **заявки** (список RLS-aware + создание + инлайн-смена статуса), **клиенты** (список + создание физлица/организации), **устройства** (список + создание). Действия гейтятся по ролям на клиенте (создание — admin/manager, статус — +engineer) поверх серверного RBAC/RLS. Dev-прокси `/api → :8080`. Проверено: сборка `tsc+vite` зелёная; цикл приёмки (клиент→устройство→заявка→статус) прошёл end-to-end через API стенда. Дальше: склад, счета, сотрудники, деталь заявки, назначение исполнителя, редактирование.
